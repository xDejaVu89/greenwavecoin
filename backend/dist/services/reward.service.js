...
exports.finaliseEpoch = finaliseEpoch;
exports.markEpochPublished = markEpochPublished;
exports.markEpochFailed = markEpochFailed;
exports.getClaimsForWallet = getClaimsForWallet;
...
// ---------------------------------------------------------------------------
const stmts = {
    insertEpoch: database_1.db.prepare(`INSERT OR IGNORE INTO reward_epochs (epoch, merkle_root, total_gwc, claim_count, status)
     VALUES (@epoch, @merkle_root, @total_gwc, @claim_count, 'pending')`),
    updateEpochPublished: database_1.db.prepare(`UPDATE reward_epochs SET status = 'published', tx_hash = @tx_hash,
     published_at = unixepoch() WHERE epoch = @epoch`),
    updateEpochFailed: database_1.db.prepare(`UPDATE reward_epochs SET status = 'failed' WHERE epoch = @epoch`),
    insertClaim: database_1.db.prepare(`INSERT OR IGNORE INTO reward_claims
       (epoch, claim_index, wallet, amount_wei, proof)
     VALUES (@epoch, @claim_index, @wallet, @amount_wei, @proof)`),
    getClaimsByWallet: database_1.db.prepare(`SELECT rc.epoch, rc.claim_index, rc.wallet, rc.amount_wei, rc.proof, rc.claimed,
    ...
    getLatestEpoch: database_1.db.prepare(`SELECT MAX(epoch) as max_epoch FROM reward_epochs`),
    getEpoch: database_1.db.prepare(`SELECT * FROM reward_epochs WHERE epoch = ?`),
    markClaimed: database_1.db.prepare(`UPDATE reward_claims SET claimed = 1 WHERE epoch = ? AND LOWER(wallet) = LOWER(?)`),
};
// ---------------------------------------------------------------------------
// Merkle leaf hashing (must match RewardEscrowV2.sol)
// ---------------------------------------------------------------------------
function hashLeaf(index, wallet, amountWei) {
    // keccak256(abi.encodePacked(index, account, amount))
    const encoded = ethers_1.ethers.solidityPacked(['uint256', 'address', 'uint256'], [index, wallet, amountWei]);
    return Buffer.from(ethers_1.ethers.keccak256(encoded).slice(2), 'hex');
}
// ---------------------------------------------------------------------------
 ...
 */
function calculateRewards(sinceTimestamp, untilTimestamp) {
...
function buildMerkleTree(rewards) {
    ...
    const tree = new merkletreejs_1.MerkleTree(leaves, (data) => Buffer.from(ethers_1.ethers.keccak256(data).slice(2), 'hex'), { sort: true });
 ...
 */
function finaliseEpoch(epoch, sinceTimestamp, untilTimestamp) {
    const existing = stmts.getEpoch.get(epoch);
    if (existing) {
 ...
 */
function markEpochPublished(epoch, txHash) {
    stmts.updateEpochPublished.run({ epoch, tx_hash: txHash });
}
function markEpochFailed(epoch) {
    stmts.updateEpochFailed.run(epoch);
}
/**
 * Get reward claims for a specific wallet address.
 */
function getClaimsForWallet(wallet) {
    const rows = stmts.getClaimsByWallet.all(wallet);
    return rows.map(row => ({
        epoch: row.epoch,
...
function getNextEpoch() {
    const row = stmts.getLatestEpoch.get();
